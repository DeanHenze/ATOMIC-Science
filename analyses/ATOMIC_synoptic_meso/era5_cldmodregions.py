...
def createfiles(path_era5dir, path_cldmodtab, path_savedir, 
                dlat=2, dlon=2, dt_before=3.1, dt_after=0):
    """
    Get ERA5 data for latitude, longitude, and time windows centered on P-3 
    mean location / time of cloud module sampling.
    ...
    dlat, dlon, dt: scalars.
        Window half width of latitude and longitude (degrees) to get data for.
        
    dt_before, dt_after: scalars.
        Time interval before and after cloud module (hours) to get data for.
    """
    ...
    dt_before = pd.Timedelta(hours=dt_before)
    dt_after = pd.Timedelta(hours=dt_after)
    ...
    for i, row in cldmod_tab.iterrows():
        ...
        era5_cldmod = era5.sel(
            ...
            time = slice(tmean-dt_before, tmean+dt_after)
            )
