# -*- coding: utf-8 -*-
"""
Created on Fri Jul  8 10:16:02 2022
...
from scipy.odr import Model, RealData, ODR # 1.1.0
from scipy.optimize import curve_fit # 1.1.0
from scipy.stats import iqr # 1.1.0
from scipy.integrate import dblquad # 1.1.0
import matplotlib.pyplot as plt # 2.0.2
import xarray as xr # 0.12.3




def oversample_1d(mapdata, xcoords, gridx, ffact=1., return_stdev='no'):
    """
    ...
    def weights_gauss(xdist, sigx):
        A_norm = 1/(sigx*np.sqrt(2*np.pi)) # Normalization factor
        return A_norm*np.exp(-0.5*(xdist**2/sigx**2))    
        
    k = len(gridx)
    map_ovrsmpled = np.ones(k)*np.nan
    std_map = np.ones(k)*np.nan
    n_obs_weighted = np.ones(k)*np.nan
    ...
    for j in range(k):
        ...
        # Gaussian-distance weights:
            ...
            w_gauss = weights_gauss(xdist, hx)
        
        # Weighted total number of obs:
            n_obs_weighted[j] = np.sum(w_gauss)
        
        # Weighted mean:
            S = np.sum(w_gauss)
            map_ovrsmpled[j] = np.sum(mapdata*w_gauss)/S
        ...
        # won't matter for a large number of data points:
            if return_stdev == 'no':
                ...
                std_map[j] = (np.sum(w_gauss*(wmean_ij - mapdata)**2)/S)**0.5
    
            elif return_stdev == 'yes_addweights': 
                wmean_ij = map_ovrsmpled[j]
                std_map[j] = (np.sum(w_gauss*(wmean_ij - mapdata)**2)/S)**0.5                
         

    ## Collect results in dict and return:
    results = {'mean':map_ovrsmpled, 'stdev':std_map, 'bandwidths':hx, 
...
def oversample_2d(mapdata, xcoords, ycoords, gridx, gridy, 
