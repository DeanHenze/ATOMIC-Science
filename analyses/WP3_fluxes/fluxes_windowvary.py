...
Extension of the 'fluxes.py' script for varying window sizes. Fluxes 
for each cloud module saved with different window sizes as different files.
"""
...
dir_fluxes = "./fluxes_levlegs_windowvary/"
if not os.path.isdir(dir_fluxes):
    os.makedirs(dir_fluxes)


# All cospectra filenames:
dir_wcospec = "./cospectra_levlegs_windowvary/"
...
# All unique cospectra windowing cases:
windows = [f[i1:i2] for f, i1, i2 
           in zip(
               fnames_wcospec, 
               [f.index('_window') + 1 for f in fnames_wcospec], 
               [f.index('.nc') for f in fnames_wcospec]
               )
          ]
...
for n in ncldmod_unique:
    ...
    for win in windows_unique:
    
        print("Working on cloud module # %s, %s" % tuple([n, win]))
        
        fnames_cldmod = [f for f in fnames_wcospec 
                         if (("_cld%s" % n) in f) and (("_%s" % win) in f)]
        ...
        flux_ds.to_netcdf(
            dir_fluxes + 
            "WP3_%s_cld%s_fluxes_%s.nc" % tuple([date, n, win])
            )
